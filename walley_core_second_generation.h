 ...
 * Author: shd101wyy
 *
 * Created on September 2, 2012, 3:44 PM
 */
...
printf("Begin to Run While\n");
            ...
            while (Walley_Judge_With_And_And_Or_With_Parenthesis_And_Variables_Function_Second_Generation(last_while_sentence2, file_name) == TRUE) {
                ...
                Walley_Run_Second_Generation(file_name, setting_file, temp_file_name, existing_file, temp_to_run);
        ...
        if (current_space > space_of_first_for_sentence && current_space % 4 == 0) {
            ...
            **/
                ...
                if(strcmp(trim(temp_to_run),"continue")==0){
                    ...
                    Walley_Run_Second_Generation(file_name, setting_file, temp_file_name, existing_file, init_temp_i);
                    Walley_Run_Second_Generation(file_name, setting_file, temp_file_name, existing_file, temp_to_run);
    ...
    if(can_run_basic_input == TRUE && (numOfSpaceAheadString(input_str) == 0 || space==current_space)){
        /*
                ...
                if (removeAheadSpace(input_str)[0] == '#') {
                    ...
                    if (find(removeAheadSpace(input_str), "if ") == 0) {
                        ...
                        can_run = Walley_Judge_With_And_And_Or_With_Parenthesis_And_Variables_Function(sentence, file_name);
                        ...
                        can_run = Walley_Judge_With_And_And_Or_With_Parenthesis_And_Variables_Function(last_if_sentence, file_name);
                        printf("can_run is %d\n",can_run);
                        if (can_run == 1)
                            can_run = 0;
                        else
                    ...
                    bool can_run_while = Walley_Judge_With_And_And_Or_With_Parenthesis_And_Variables_Function(last_while_sentence, file_name);
                    ...
                    in_what = Walley_Substitue_Var_And_Function_Return_Value_From_File(in_what, file_name);
        ...
        if (input_temp[0] == '#' ||
            ...
            //#####################  println  ###################
            if (find_not_in_string(input_str, "println") == 0) {
                char* output = Walley_Println(file_name, strInBrackets(input_str));
                printf("%s", output);
            }//#####################  print  ###################
            else if (find_not_in_string(input_str, "print") == 0) {
                char* output = Walley_Print(file_name, strInBrackets(input_str));
                printf("%s", output);
            }                //#################### import ####################
                //#################### import ####################
                ...
                Walley_Run_Second_Generation(file_name, setting_file, temp_file_name, existing_file, content_of_file);
            ...
            //This is also import
               ...
               //printf("content of file is |%s|\n", content_of_file);
                ...
                Walley_Eval_And_Update_Var_And_Value_To_File(file_name, input_str);
            } else {
                ...
                if (can_just_eval) {
                    ...
                    input_str = Walley_Substitue_Var_And_Function_Return_Value_From_File(input_str, file_name);
    ...
    if (continue_run == TRUE && run_goto == FALSE) {
        ...
        if (find_gang_gang == TRUE) {
            Walley_Run_Second_Generation(file_name, setting_file, temp_file_name, existing_file, substr(temp_input_str, find(temp_input_str, "\\n") + 2, (int) strlen(temp_input_str)));
...
char *Walley_Run_One_Function_And_Return_Value_Second_Generation(char *input_str,char *get_var_from_file_name){
    ...
    while ((fgets(arr, 1000, fp)) != NULL) {
        ...
        if(begin==TRUE&&finish_init_param==FALSE){
            ...
            if ((int) strlen(removeAheadSpace(removeBackSpace(arr_arr))) != 0){
                //printf("Begin to run\n");
                printf("|%s|, |%s|,|%s|,|%s|,|%s|\n",file_var_temp_name,file_settings_temp_name,file_file_temp_name,"__walley_function__.wy",arr_arr);
                printf("ENTER HERE\n");
                
                Walley_Run_Second_Generation(file_var_temp_name, file_settings_temp_name,file_file_temp_name,"__walley_function__.wy",arr_arr);
            }
    ...
    if(strcmp(return_var_name,"None")==0){
        ...
        return_var_name=Walley_Substitue_Var_And_Function_Return_Value_From_File(return_var_name,file_var_temp_name);
    ...
    for(i=0;i<(int)strlen(input_str);i++){
        if(charIsInString(input_str,i)==FALSE && input_str[i]=='{'){
            ...
            if(finishFindingVarAndFunction(replace_str)==FALSE){
                char *with_str=Walley_Substitue_Var_And_Function_Return_Value_From_File_Second_Generation(replace_str,file_var_name);
...
void Walley_Eval_And_Update_Var_And_Value_To_File_Second_Generation(char* file_var_name,char* input_str) {
    ...
    if(find_not_in_string(var_name,".")!=-1){
        char *user=substr(var_name,0,find_not_in_string(var_name,"."));
        if(checkWhetherSameInstanceExisted("__walley__.wy",user)==TRUE){
            Walley_Eval_And_Update_Var_And_Value_To_File_Second_Generation("__walley__.wy",input_str);
        }
    }**/
}
